...
export class AxiosApiService implements ApiService {
  ...
  public async toRestaurant(
    request: CreateRestaurantRequest,
  ): Promise<Restaurant> {
    const res=request as unknown as Restaurant;
    res.photos=request.photoIds ?.map((id) => ({
      id,
      caption: "",
      url: `/api/photos/uploads/${id}`,
  ...
  public async createRestaurant(
    ...
    // Convert request to Restaurant type
    const res = await this.toRestaurant(request);
