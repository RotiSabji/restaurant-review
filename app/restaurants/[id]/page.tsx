...
const filteredNearby = restaurantsNearBy.content.filter(
  ...
  const getImageUrl = (restaurant: Restaurant) => {
    if (
      ...
      const filename = restaurant.photos[0].url;
      // Try static serving first
      const staticUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/${filename}`;
      // Optionally, you can check if the static file exists, but for now just return the static path
      
      return staticUrl;
      // If you want to fallback to API route, you could add logic here
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-4">
                    <div className="flex gap-2">
                      <p className="text-sm">{review.writtenBy?.username || "Unknown user"}</p>
